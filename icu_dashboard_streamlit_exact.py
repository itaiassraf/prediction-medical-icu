...
options = [{'label': desc, 'value': var} for var, desc in categories[category].items()]
var_desc = dict(options)[options[0]["value"]]  # default label
selected_var = st.selectbox("בחר משתנה", options, format_func=lambda x: x['label'])['value']
...
fig.update_layout(
    barmode='overlay',
    title=f"<b>{categories[category][selected_var]}</b>",
    plot_bgcolor='#1c1e29',
